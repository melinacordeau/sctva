...
def select_interTVA_fibres(path_input_tracts, path_functional_peaks_coord, radius=5):
    ...
    for i, p1 in enumerate(sorted_peaks):
        for j in range(i, len(sorted_peaks)):
            p2 = sorted_peaks[j]
            path_selected_tracts = os.path.join(
                os.path.basename(path_input_tracts), str(i) + "-" + str(j) + ".tck"
            )
            select_tracks_between_peaks(
                path_input_tracts, path_selected_tracts, p1, p2, radius
            )
    pass
